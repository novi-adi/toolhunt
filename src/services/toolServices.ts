...
export const toolServices = {
    getData: ({ isPublic, q }: { isPublic?: boolean, q?: string }) => {
        ...
        // jika true, tambahkan filter, jika tidak kosongkan
        ...
        const allTools = client.query(sql).all(`%${query}%`, `%${query}%`) as ITool[];
    ...
    createData: ({ name, description, image, url, category}: Omit<ITool, "id" | "visits" | "verified" | "isPublic">) => {
        ...
        client.query(`INSERT INTO analytics (id, tool_id) VALUES (?,?)`).run(analyticId, toolId);

        const currentTool = client.query(`
            SELECT
                t.id AS id,
                t.name AS name,
                t.description AS description,
    ...
    updateAnalytics: (id:string) => {
        client.query(`
            UPDATE analytics SET visits = visits + 1 WHERE tool_id = ?
        `).run(id);
